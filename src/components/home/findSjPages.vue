<template>
	...
	<div class="allTop">
		...
		<div class="box_middle">
			<div class="row">
				<el-form  class="demo-form-inline" :inline="true" label-width="100px" label-position="">
					  <el-form-item label="着陆页样式">
					  ...
					  <el-form-item label="着陆页名称">
					    <el-input placeholder="请输入着陆页名称 " v-model.trim='description'  style="width: 200px;"></el-input>
					  </el-form-item>
					  <el-form-item label="客户名称">
					   	 <el-input placeholder="请输入客户名称" v-model.trim='clientName'style="width: 200px;"></el-input>
					  </el-form-item>
					  <el-form-item >
			...
			<div class="ihomeBoxM_table " v-show="!none">
				<el-table :data="topdata" v-loading="loading">
				    ...
				    <el-table-column  prop="clientName" label="客户名称">
				     ...
				     <el-table-column  prop="storagePath" label="查看">
				        <template slot-scope="scope">
					      	<a v-bind:href="hrefFun(scope.row.storagePath)" target="_blank">预览</a>
					    </template>
				    </el-table-column>
			    </el-table>
			</div>
...
export default {
  ...
  computed: {
	...
	methods:{
		hrefFun(hre){
			let Url=hre
			Url=Url.replace("/mnt/local","").replace("/usr/local","").replace("-D","")
			return 'http://ad.bjzgxr.net'+Url
		},
	  	number(val){
			var val=val*100
			if(val!=0){
		...
		imgBG(index){
	 	...
	 	dataSearch(){
	 		this.loading=true
	 		console.log()
	 		this.$xhr.get("/pageShow/findSjPages",{
				 params: {
