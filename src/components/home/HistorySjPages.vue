...
export default {
  ...
  computed: {
	...
	mounted(){
		...
		this.name=this.$router.currentRoute.params.seeAllT
	...
	methods:{
		...
		imgBG(index){
	 	...
	 	dataSearch(){
	 		this.loading=true
	 		this.$xhr.get("/pageShow/findHistorySjPages",{
				 params: {
				 	modelId:this.mStyle,//模板id
				 	clientName:this.clientName,//客户名称
				 	description:this.description,//关键词
				 	channelId:this.channel,//分公司id
				 	currPage:this.page,//当前页数
				 }
			}).then((res)=>{
